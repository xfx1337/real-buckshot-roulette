...
#
# Ключи каналов: "game" (SoundEngine на панели дилера), "video" (видео/CCTV
# на TV-экране) и "phone" (капсюль телефонной трубки).
#
# Телефон только серверный: капсюль подключён к ноутбуку кабелем, а звук в него
# отправляет Python (voip/scripts/audio.py), не браузер. В браузерном списке он
# всё равно есть — так оба списка остаются одним набором каналов, — но выбор там
# ни на что не влияет.
_OUTPUTS_KEY = "__outputs__"
OUTPUT_CHANNELS = ("game", "video", "phone")
