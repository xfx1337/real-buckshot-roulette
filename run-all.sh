...
#   --no-farm       не проверять связь с машиной, где клонируются голоса
...
# добавлять новые голоса из панели, нужна машина с видеокартой; связь с ней
# идёт по ssh отдельной командой на каждый запрос, поднимать ничего не надо.
...
# Машина с видеокартой, на которой клонируются голоса. Дотягиваемся до неё
# обычным ssh — по одной команде на запрос, без постоянного канала. Имя хоста
# берётся из ~/.ssh/config; переопределяется переменной окружения, если ферма
# переехала.
...
" 2>/dev/null || echo "")
  ...
  FARM_GPU=$(echo '{}' | ssh -o BatchMode=yes -o ConnectTimeout=6 "$FARM_HOST" \
    "cd ${TTS_FARM_DIR:-~/backshot-tts} && ${TTS_FARM_PYTHON:-venv/bin/python} -m tts.remote health" \
    2>/dev/null | sed -n 's/.*"gpu": *"\([^"]*\)".*/\1/p')
  [[ -n "$FARM_GPU" ]] \
    && ok "ферма голосов: $FARM_GPU" \
    || note "ферма голосов не отвечает (новые голоса добавить нельзя)"
...
do_stop() {
  ...
  # Фермы здесь нет в списке намеренно: с неё нечего гасить. Постоянного
  # соединения к ней не держится, а генерация на той машине живёт своей жизнью
  # и переживает остановку этого Mac.
...
#
# Какой голос говорит, если никто не сказал иначе. Без этого движок берёт свой
# «default», которого на диске нет: проверка ниже нашла бы установленный голос
# и всё равно доложила о нехватке всех фраз до единой, потому что смотрела бы
# в пустой каталог. Панель дилера по-прежнему переключает голос на ходу, а
# TTS_VOICE из окружения имеет приоритет над этой строкой.
export TTS_VOICE="${TTS_VOICE:-mariarti}"
VOICE_STATE=$("$PY" -c "
import tts
s = tts.engine.available()
print(s['voice'], s['phrases'], s['expected'], int(bool(s['voices'])))
...
if [[ "$DO_FARM" -eq 1 ]]; then
  ...
  # Одна короткая команда по ssh, и всё. Туннеля больше нет: панель ходит на
  # ферму отдельным соединением на каждый запрос, так что поднимать и стеречь
  # здесь нечего. Раньше в этом месте жил проброс порта и сторож при нём —
  # ssh переживал разрыв сети, продолжая слушать локальный порт, и запросы в
  # такой канал висели до таймаута. Панель выглядела сломанной, а не
  # отключённой, и починка была ручной работой каждый вечер.
  #
  # Проверка поэтому спрашивает саму ферму, а не порт: единственный признак,
  # которому можно верить, — это ответ.
  # ssh -G разворачивает ~/.ssh/config. Если hostname остался равен самому
  # имени, записи о такой машине нет и подключаться некуда.
  if ssh -G "$FARM_HOST" 2>/dev/null | grep -q "^hostname $FARM_HOST$"; then
    warn "хост «${FARM_HOST}» не описан в ~/.ssh/config"
    note "Без него вкладка «Голоса» скажет, что ферма не настроена"
  else
    FARM_GPU=$(echo '{}' | ssh -o BatchMode=yes -o ConnectTimeout=8 "$FARM_HOST" \
      "cd ${TTS_FARM_DIR:-~/backshot-tts} && ${TTS_FARM_PYTHON:-venv/bin/python} -m tts.remote health" \
      2>/dev/null | sed -n 's/.*"gpu": *"\([^"]*\)".*/\1/p')
    if [[ -n "$FARM_GPU" ]]; then
  ...
  # Адрес фермы для игрового сервера — ssh-хост, а не URL.
  export TTS_FARM_HOST="$FARM_HOST"
fi
